...
from src.exceptions import UserPasswordToShortHTTPException, UserAllReadyExistsHTTPException, \
    ObjectAlreadyExistsException, UserNotRegisterHTTPException, WrongPasswordHTTPException, TokenWrongTypeHTTPException, \
    ExpiredSignatureErrorHTTPException, PyJWTErrorHTTPException, \
    WrongRefreshTokenHTTPException, RefreshTokenRequiredHTTPException, \
...
class AuthService(BaseService):
    ...
    async def store_refresh_token(self, user_id: int, refresh_token: str):
        ...
        await redis_manager_auth.set(key, refresh_token, expire=timedelta(days=settings.REFRESH_TOKEN_EXPIRES_DAYS))
        await redis_manager_auth.set(rt_key, str(user_id), expire=timedelta(days=settings.REFRESH_TOKEN_EXPIRES_DAYS))
    ...
    async def register_user(self, data: UserRequestAddDTO):
        ...
        new_user_data = UserAddDTO(name=data.name, sname=data.sname, age=data.age, email=data.email, hashed_password=hashed_password)
        try:
